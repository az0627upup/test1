...
def create_BTree_By_List(array):
    ...
    if len(level_order) == 1:
        return BTree(level_order[0][0])    #如果len(level_roder)为1的话说明只有一个节点，那这个节点的值为l[0][0]而不是l[0]
    else:    # 二叉树的层数大于1

        # 创建最后一层的节点列表
        BTree_list = [BTree(elem) for elem in level_order[-1]]  #取出最后一层中的每个元素
...
#  利用Graphviz进行二叉树的可视化
# tree.print_tree(save_path='E://test//create_btree_by_list1.gv', label=True)
